                        ...
                        <table class="table table-striped table-bordered table-hover example">
                            <thead>
                                <tr>
                                        ...
                                        <th>آدرس</th>
                                        <th>شماره تماس</th>
                                        <th>آخرین تاریخ مراجعه</th>
                                        <th style="width:100px; text-align: center;">ملاحضات</th>
                                </tr> 
                            </thead>
                            <tbody>
                                <?php
                                if (empty($resultlist)) {
                                    ...
                                    foreach ($resultlist as $report) {
                                        ...
                                        if (!($this->rbac->hasPrivilege('opd_patient', 'can_view')) && ($patient_type == "Outpatient")) {
                                            ...
                                            ?>      
                                            <tr> 
                                                <td><?php echo $report['hmis_no']; ?></td>
                                                <td>
                                                    <a target="_blank" href="<?php echo $url; ?>"><?php echo $report['guardian_name'] ?>&nbsp; <?php echo $report['patient_name'] ?>
                                                    </a>
                                                </td>
                                                <td><?php echo $report['age']; ?></td>
                                                <td><?php echo $report['gender']; ?></td>
                                                <td><?php echo $report['address']; ?>,<?php echo $report['district']; ?>,<?php echo $report['province']; ?></td>
                                                <td><?php echo $report['mobileno']; ?></td>
                                                <td><?php echo date($this->customlib->getSchoolDateFormat(true, true), strtotime($report['created_at'])) ?></td>
                                                <td class="text-right">
                                                    <a href="<?php echo $url ?>" data-toggle="tooltip" target="_blank" class="btn btn-default btn-xs" title="نمایش جزییات " >
